    ...
    try:
        if a.command=="inspect": code=_inspect(a.episode)
            ...
            if a.command=="doctor": code=_doctor(cfg,replay=a.replay,freedrive=a.freedrive)
                ...
                if a.move_to_fixed_start and not a.execute:raise ValueError("--move-to-fixed-start requires --execute")
                ...
                code=run_recording(cfg,a.instruction,a.execute,a.initial_gripper,a.enable_freedrive,a.return_to_start,a.return_to_start_duration_sec,a.move_to_fixed_start)
                ...
                if a.return_to_start_duration_sec<=0:raise ValueError("--return-to-start-duration-sec must be positive")
                if a.smoothing_window_sec<=0 or a.gripper_anchor_window_sec<0 or a.smoothing_polyorder<1:raise ValueError("invalid smoothing parameters")
                code=run_replay(Path(a.episode),cfg,a.execute,a.wall_clock_gripper_fallback,a.move_to_start,a.move_to_start_duration_sec,a.return_to_start,a.return_to_start_duration_sec,a.smooth_trajectory,a.smoothing_window_sec,a.smoothing_polyorder,a.gripper_anchor_window_sec)
